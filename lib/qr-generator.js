...
export async function generateQRCard(guest, token) {
  ...
  const T = isArabic
    ? { title: 'تصريح دخول',      subtitle: 'للاستخدام مرة واحدة',
        lDate: 'التاريخ',          lGuests: 'عدد المدعوين', lVisitor: 'المدعو',
        lKids: 'الأطفال',          noKids: 'غير مسموح',  kidsUnit1: 'طفل',  kidsUnitN: 'أطفال',
        kidsNote: '* لا يُسمح بدخول الأطفال تحت سن 15 عاماً', dir: 'rtl' }
    : { title: 'INVITATION PASS', subtitle: 'Single Use Pass',
        lDate: 'Date',             lGuests: 'Allowed Guests', lVisitor: 'Guest',
        lKids: 'Kids',             noKids: 'Not Allowed',     kidsUnit1: 'Kid',  kidsUnitN: 'Kids',
        kidsNote: '* Children under 15 are not permitted',    dir: 'ltr' };
  ...
  const INFO_Y = 463, INFO_H = 166, ROW1_Y = 487, ROW2_Y = 527, ROW3_Y = 567, ROW4_Y = 607;
  ...
  const textSvg = `<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    ${infoRow(ROW3_Y, T.lKids,    kidsValue)}
    ${infoRow(ROW4_Y, T.lVisitor, guestName)}
    ${!kidsAllowed ? `<text x="250" y="660" ${F} font-size="11" fill="#cc3300" text-anchor="middle" direction="${T.dir}">${xml(T.kidsNote)}</text>` : ''}
  </svg>`;
